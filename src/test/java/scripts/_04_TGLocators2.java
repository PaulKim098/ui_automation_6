...
public class _04_TGLocators2 extends GlobalBase {
    ...
    public void validateLanguages(){
        ...
        for (int i = 0; i <= 2; i++) {
            ...
            Assert.assertEquals(languages.get(i).getText(), expectedLanguages[i]);
        }
    }
    ...
    public void validateTools(){
        List<WebElement> tools = driver.findElements(By.xpath("(//h3[@class='Xpath_subheader__VOtsU'])[2]/../ul/li"));

        String[] expectedTools = {"Selenium WebDriver", "WebDriverIO", "Cypress", "Playwright"};

        for (int i = 0; i <= 3; i++) {
            Assert.assertTrue(tools.get(i).isDisplayed());
            Assert.assertEquals(tools.get(i).getText(), expectedTools[i]);
        }
    }
    ...
    public void validateListHeaders(){
        ...
        for (int i = 0; i < listHeaders.size(); i++) {
            ...
            Assert.assertEquals(listHeaders.get(i).getText(), expectedHeaders[i]);
        }
